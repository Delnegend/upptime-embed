...
const colorScheme = ref<"dark" | "light">(configuredColorScheme === "dark" ? preferredColorScheme : configuredColorScheme);
		...
		<div
			...
			<div class="ml-auto grid w-full max-w-72 grid-cols-5 gap-2">
				<Button
					...
					size="xs"
					variant="secondary"
					class="bg-card shadow transition-all"
			...
			<div
				...
				<div class="flex flex-col justify-center gap-2">
					<div
						:class="cn(
							...
							colorScheme === 'dark' && {
								...
								'animate-bounce text-red-500': detail.status === 'down',
								'text-yellow-400': detail.status === 'degrade',
								'text-gray-400': detail.status === 'unknown',
							},
							colorScheme === 'light' && {
								'text-green-600': detail.status === 'up',
								'animate-bounce text-red-600': detail.status === 'down',
								'text-yellow-600': detail.status === 'degrade',
					...
					<div class="flex gap-2 text-xs text-secondary-foreground/70">
						<code>🟢 {{(() => {
							switch (viewDuration) {
